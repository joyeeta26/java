...
public class multipledaigonalwitharraylist {
	...
	public static ArrayList<String> getBoardPaths(int cc, int cr, int ec, int er) {
		...
		if (cc < ec&&cr<er)
			{for(int t=1;t<ec&&t<er;t++ )
			{ArrayList<String> recResult = getBoardPaths(cc + t, cr+t, ec, er);
			for (int i = 0; i < recResult.size(); i++) {
				myResult.add(recResult.get(i) + "D");
			}
		}}
		return myResult;
	}
